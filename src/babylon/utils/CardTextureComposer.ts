...
export class CardTextureComposer {
   ...
   * High resolution (2048x3072) with optimized contrast for text clarity
   */
  public async createCardTexture(card: Card, width: number = 2048, height: number = 3072): Promise<DynamicTexture> {
    ...
    if (portraitImage) {
      ...
      // Draw portrait with rounded corners effect
      ctx.save();
  ...
  private drawValue(ctx: CanvasRenderingContext2D, value: number, width: number, height: number): void {
    ...
    // Draw background circle with high contrast
    ctx.fillStyle = 'rgba(0, 0, 0, 0.95)'; // Increased opacity for better contrast
    ctx.beginPath();
    ...
    // Draw value text with shadow for depth
  ...
  private drawName(ctx: CanvasRenderingContext2D, name: string, width: number, height: number): void {
    ...
    // Draw opaque background banner for maximum contrast
    ctx.fillStyle = 'rgba(0, 0, 0, 0.9)'; // Increased opacity from 0.6 to 0.9
    ctx.fillRect(width * 0.2, y - 100, width * 0.6, 200);

    // Draw name with thick outline for visibility
   ...
   * High contrast backgrounds and thick outlines for maximum readability
   */
  private drawAbility(ctx: CanvasRenderingContext2D, ability: string, width: number, height: number): void {
    ...
    // Draw FULLY OPAQUE background for maximum contrast
    ctx.fillStyle = 'rgba(0, 0, 0, 0.95)'; // Nearly solid black background
    ctx.fillRect(padding - 40, startY - 60, maxWidth + 80, height * 0.28);

    // Word wrap the ability text with high-quality rendering
    ctx.fillStyle = '#ffffff';
    ctx.strokeStyle = '#000000';
    ctx.lineWidth = 12; // Thicker outline for better definition
